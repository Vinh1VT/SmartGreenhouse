#ifndef SENSOR_DATA_H
#define SENSOR_DATA_H

#include <Arduino.h>
#include <cstdint>

#define HUM_EST_10 0
#define HUM_EST_30 1
#define HUM_SUD_10 2
#define HUM_SUD_30 3
#define HUM_OUEST_10 4
#define HUM_OUEST_30 5





struct SensorData{
  // --- Bloc T: Températures (int16_t) ---
  // Défaut : 32766 (0x7FFE) -> Absent
  // 32767 (0x7FFF) -> ERREUR CAPTEUR
  int16_t temp_ambiant = 0x7FFE;
  int16_t temp_puit_out= 0x7FFE;
  int16_t temp_puit_in = 0x7FFE;
  int16_t temp_terre_1 = 0x7FFE;
  int16_t temp_terre_2 = 0x7FFE;

  // --- Bloc H: Humidités (uint8_t) ---
  // Défaut : 254 (0xFE) -> Absent
  // 255 (0xFF) -> ERREUR CAPTEUR
  uint8_t hum_ambiant  = 0xFE;
  uint8_t hum_oya_1   = 0xFE;
  uint8_t hum_oya_2   = 0xFE;
  uint8_t hum_oya_3   = 0xFE;
  uint8_t hum_oya_4   = 0xFE;
  uint8_t hum_bille_1 = 0xFE;
  uint8_t hum_bille_2 = 0xFE;

  // --- Bloc L: Luminances (uint16_t) ---
  // Défaut : 65534 (0xFFFE) -> Absent
  // 65535 (0xFFFF) -> ERREUR CAPTEUR
  uint16_t lum_ambiant = 0xFFFE;

  // --- Bloc G: Gaz (uint16_t) ---
  // Défaut : 65534 (0xFFFE) -> Absent
  // 65535 (0xFFFF) -> ERREUR CAPTEUR
  uint16_t co2         = 0xFFFE;
  uint16_t o2          = 0xFFFE;

};


uint8_t buildPayload(const SensorData& data, uint8_t* buffer); //Genere le message depuis sensordata
void arrayToHex(uint8_t* buffer, uint8_t length,char* hexString); //HexString à envoyer a AT+MSGHEX

#endif